...
func (b *flownodeBuilder) generatePodTemplateSpec() corev1.PodTemplateSpec {
	...
	if len(b.Cluster.Spec.Flownode.Template.MainContainer.PrependArgs) > 0 {
		podTemplateSpec.Spec.Containers[constant.MainContainerIndex].Args = append(b.Cluster.Spec.Flownode.Template.MainContainer.PrependArgs, podTemplateSpec.Spec.Containers[constant.MainContainerIndex].Args...)
	}

	if len(b.Cluster.Spec.Flownode.Template.MainContainer.ExtraArgs) > 0 {
		podTemplateSpec.Spec.Containers[constant.MainContainerIndex].Args = append(podTemplateSpec.Spec.Containers[constant.MainContainerIndex].Args, b.Cluster.Spec.Flownode.Template.MainContainer.ExtraArgs...)
	}
	...
	if logging := b.Cluster.GetFlownode().GetLogging(); logging != nil && !logging.IsOnlyLogToStdout() {
