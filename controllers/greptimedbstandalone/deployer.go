...
func (b *standaloneBuilder) generatePVCs() []corev1.PersistentVolumeClaim {
	...
	if fs := b.standalone.GetDatanodeFileStorage(); fs != nil {
		claims = append(claims, *common.FileStorageToPVC(b.standalone.Name, fs, common.FileStorageTypeDatanode))
	}
	...
	if fs := b.standalone.GetWALProvider().GetRaftEngineWAL().GetFileStorage(); fs != nil {
		claims = append(claims, *common.FileStorageToPVC(b.standalone.Name, fs, common.FileStorageTypeWAL))
	}
	...
	if fs := b.standalone.GetObjectStorageProvider().GetCacheFileStorage(); fs != nil {
		claims = append(claims, *common.FileStorageToPVC(b.standalone.Name, fs, common.FileStorageTypeCache))
	}
