...
func Marshal(config Config, strategy v1alpha1.ConfigMergeStrategy) ([]byte, error) {
	...
	output, err := mergeConfig(tree, config, strategy)
	if err != nil {
		return nil, err
	}
...
func FromCluster(cluster *v1alpha1.GreptimeDBCluster, roleSpec v1alpha1.RoleSpec) ([]byte, error) {
	...
	return Marshal(cfg, cluster.Spec.ConfigMergeStrategy)
}
...
func FromStandalone(standalone *v1alpha1.GreptimeDBStandalone) ([]byte, error) {
	...
	return Marshal(cfg, standalone.Spec.ConfigMergeStrategy)
}

func isRemoteWALConfigKey(key string) bool {
	switch key {
	case "wal.provider",
		...
		"wal.sasl.password",
		"wal.tls",
		"wal.tls.server_ca_cert_path",
		"wal.tls.client_cert_path",
		"wal.tls.client_key_path":
		return true
	default:
		return false
	}
...
func mergeConfig(input *toml.Tree, config interface{}, strategy v1alpha1.ConfigMergeStrategy) (string, error) {
	...
	for i := 0; i < valueOf.NumField(); i++ {
		...
		// except remote WAL keys where the structured CRD config is authoritative.
		if input.Has(tag) && strategy == v1alpha1.ConfigMergeStrategyInjectedDataFirst && !isRemoteWALConfigKey(tag) {
			continue
		}

		// Handle the `,inline` field.
		if tag == InlinedFieldTag {
			_, err := mergeConfig(input, valueOf.Field(i).Interface(), strategy)
			if err != nil {
				return "", err
			}
			continue
